...
const checkoutSchema = z.object({
  ...
  guestAddressAlias: z.string().optional(),
  guestStreetName: z.string().optional(),
  guestStreetNumber: z.string().optional(),
  guestInteriorNumber: z.string().optional(),
  guestNeighborhood: z.string().optional(),
...
export default function CheckoutClientPage({ bootstrap }: { bootstrap: CheckoutBootstrapData }) {
  ...
  const form = useForm<CheckoutFormValues>({
    ...
    defaultValues: {
      ...
      isAnonymous: false,
      signature: '',
      couponCode: bootstrap.couponCode || '',
      gateway: 'stripe',
    },
    mode: 'onChange'
  });
  ...
  const watchedGuestStreetName = watch('guestStreetName');
  const watchedGuestStreetNumber = watch('guestStreetNumber');
  const watchedGuestNeighborhood = watch('guestNeighborhood');
  ...
  useEffect(() => {
    if (!isDataSettled) return;
    if (!isCartLoading && cartItemCount === 0 && !isProcessing) {
      router.push('/');
    }
  }, [isCartLoading, cartItemCount, router, isProcessing, isDataSettled]);
    ...
    try {
      ...
      const response = await apiFetch(endpoint, {
        ...
        body: JSON.stringify({
          ...
          guestInteriorNumber: isGuestCheckout ? data.guestInteriorNumber : undefined,
          guestNeighborhood: isGuestCheckout ? data.guestNeighborhood : undefined,
          guestCity: isGuestCheckout ? data.guestCity : undefined,
  ...
  if ((authLoading && !user) || !isDataSettled) {
          ...
          <FormProvider {...form}>
            <form id="checkout-form" onSubmit={handleSubmit(handleFinalizeOrder)} className="grid lg:grid-cols-12 gap-8 items-start w-full">
              <div className="lg:col-span-8 space-y-6 sm:space-y-8 w-full">
                ...
                <StepPhone isActive={activeStep === 1} setActiveStep={setActiveStep} disabled={hasValidationIssues} currentStep={activeStep} />
                <StepAddress isActive={activeStep === 2} setActiveStep={setActiveStep} setShippingCost={setShippingCost} disabled={hasValidationIssues} currentStep={activeStep} />
                <StepTimeSlot isActive={activeStep === 3} setActiveStep={setActiveStep} disabled={hasValidationIssues} currentStep={activeStep} />
